...
open class BootstrapTask @Inject constructor(@Input val type: String) : DefaultTask() {
    ...
    private fun getArtifacts(): Array<JsonBuilder> {
        ...
        project.configurations["runtimeClasspath"].resolvedConfiguration.resolvedArtifacts.forEach {
            ...
            if (it.file.name.contains("client") ||
                it.file.name.contains("runelite-api") ||
                it.file.name.contains("jshell") ||
                it.file.name.contains("runelite-patch")
	    ) {
                path = "https://raw.githubusercontent.com/Whatis69n/rsps-hosting/master/${type}/${it.file.name}"
                ...
                path += ".jar"
                    ...
                    it.file.name.contains("flatlaf-extras") ||
                    it.file.name.contains("flatlaf") ||
                    it.file.name.contains("archive-patcher-applier") ||
        ...
        artifacts.add(JsonBuilder(
                "name" to cjar.name,
                "path" to "https://raw.githubusercontent.com/Whatis69n/rsps-hosting/master/${type}/${cjar.name}",
                "size" to cjar.length(),
                "hash" to sha
        ))
